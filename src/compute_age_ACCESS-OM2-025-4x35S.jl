# qsub -I -P y99 -q hugemem -l mem=735GB -l storage=scratch/gh0+scratch/y99+scratch/p66 -l walltime=02:00:00 -l ncpus=24
...
# Coarsen 4x4 North of 35°S (so effectively 0.25° -> 1°)
...
agecube = DimensionalData.rebuild(volcello_ds["volcello"];
    ...
    dims = dims(volcello),
    metadata = Dict(
        "description" => "steady-state mean age with coarsening 4x4 north of 35°S",
        "model" => model,
...
# Save to netCDF file
outputfile = joinpath(inputdir, "steady_age_$(κVdeep_str)_$(κH_str)_$(κVML_str)_OM2-025-4x35S.nc")
