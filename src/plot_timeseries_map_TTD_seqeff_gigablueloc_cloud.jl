# qsub -I -P y99 -q express -l mem=47GB -l storage=scratch/gh0+scratch/xv83+gdata/fs38 -l walltime=02:00:00 -l ncpus=12
# qsub -I -P y99 -q normal -l mem=47GB -l storage=scratch/gh0+scratch/xv83+gdata/fs38 -l walltime=02:00:00 -l ncpus=12
...
cloudsourceshapefile = "/scratch/xv83/TMIP/data/ACCESS-ESM1-5/rebmeced_stnirptoof.shp"
shp_table = Shapefile.Table(cloudsourceshapefile)
cloudsourcelocations = shp_table.geometry
# Find grid cells containing source locations and count number of cloud points in each grid cell
@time cloudsourceijs = [Tuple(argmin(map(P -> norm(P .- (src_P.x, src_P.y)), zip(lon, lat)))) for src_P in cloudsourcelocations]
...
@info "Bounty Trough locations"
sourcesBountyTrough = [
    (179.6072, -46.1690)
    (179.5553, -44.4499)
]
BountyTroughfraction = [80.0u"percent", 10.0u"percent"]
# Print locations and depths (and counts 80% vs 10%?)
troughsourceijs = [Tuple(argmin(map(P -> norm(P .- src_P), zip(lon, lat)))) for src_P in sourcesBountyTrough]
troughdf = DataFrame(
    lon = Float64[],
    lat = Float64[],
    depth = Int[],
    deptho = Int[],
    fraction = typeof(1.0u"percent")[],
    src_i = Int[],
    src_j = Int[],
    src_k = Int[],
)
for (src_ij, fraction) in zip(troughsourceijs, BountyTroughfraction)
    src_i, src_j = src_ij
    src_k = findlast(wet3D[src_i, src_j, :])
    bottomcelldepth = (sum(volcello[src_i, src_j, 1:src_k]) - volcello[src_i, src_j, src_k] / 2) / areacello[src_i, src_j]
    bottomcellthickness = volcello[src_i, src_j, src_k] / areacello[src_i, src_j]
    bottomcelltopandbottom = bottomcelldepth .+ [-bottomcellthickness / 2, bottomcellthickness / 2]
    row = (
        lon = round(lon[src_i, src_j], digits=2),
        lat = round(lat[src_i, src_j], digits=2),
        depth = round(Int, bottomcelltopandbottom[2]),
        deptho = round(Int, deptho[src_i, src_j]),
        fraction = fraction,
        src_i = src_i,
        src_j = src_j,
...
#     end
# end
Γouts1trough = map(members) do member
    @info "loading $member Γ†"
    Γout_file = "/scratch/xv83/TMIP/data/$model/$experiment1/$member/$(time_window1)/cyclomonth/reemergence_time$(upwind_str)_$(κVdeep_str)_$(κH_str)_$(κVML_str).nc"
    Γoutyr4D_ds = open_dataset(Γout_file)
    map(eachrow(troughdf)) do row
        src_i, src_j, src_k = row.src_i, row.src_j, row.src_k
        Γoutyr0D = mean(readcubedata(Γoutyr4D_ds.adjointage[src_i, src_j, src_k, :]))
    end
end
Γouts1trough = cat(Γouts1trough..., dims = 2)
...
@time "Grab 2030s TTDs at injection location" 𝒢s1trough = map(eachrow(troughdf)) do row
    src_i, src_j = row.src_i, row.src_j
    readcubedata(𝒢_ds.calgtilde[src_i, src_j, :, :]).data
end
𝒢s1trough = cat(𝒢s1trough..., dims = 3)

TTD_time = collect(axes(𝒢s1trough, 1)) # saved every year so index should be same as year
...
@time "Grab 2030s seq. eff. at injection location" ℰs1trough = map(eachrow(troughdf)) do row
    src_i, src_j = row.src_i, row.src_j
    readcubedata(ℰ_ds[varname][src_i, src_j, :, :]).data
end
ℰs1trough = cat(ℰs1trough..., dims = 3)
...
# Colorbar(f[2,2], sc; vertical = false, flipaxis = false)
# translate!(cloudsc, 0, 0, 99)
...
vspan!(panelb, minimum(Γouts1trough[1, :]), maximum(Γouts1trough[1, :]), color = (troughcolors[1], 0.2))
vspan!(panelb, minimum(Γouts1trough[2, :]), maximum(Γouts1trough[2, :]), color = (troughcolors[2], 0.2))
...
𝒢2030stroughmin = ustrip.(kyr^-1, dropdims(minimum(𝒢s1trough, dims = 2), dims = 2) * s^-1)
𝒢2030stroughmax = ustrip.(kyr^-1, dropdims(maximum(𝒢s1trough, dims = 2), dims = 2) * s^-1)
# troughweights = [0.1, 0.8] # Don't think I need these
𝒢2030stroughmean = ustrip.(kyr^-1, dropdims(mean(𝒢s1trough, dims = 2), dims = 2) * s^-1)
bd2030strough1 = band!(panelb, TTD_time, 𝒢2030stroughmin[:, 1], 𝒢2030stroughmax[:, 1]; color = troughcolors[1], alpha = 0.5)
ln2030strough1 = lines!(panelb, TTD_time, 𝒢2030stroughmean[:, 1]; color = troughcolors[1], linewidth = 2, linecap = :round, joinstyle = :round)
bd2030strough2 = band!(panelb, TTD_time, 𝒢2030stroughmin[:, 2], 𝒢2030stroughmax[:, 2]; color = troughcolors[2], alpha = 0.5)
ln2030strough2 = lines!(panelb, TTD_time, 𝒢2030stroughmean[:, 2]; color = troughcolors[2], linewidth = 2, linecap = :round, joinstyle = :round)

# max𝒢s1cloud = ustrip.(kyr^-1, maximum(𝒢s1cloud) * s^-1)
max𝒢s1trough = ustrip.(kyr^-1, dropdims(maximum(𝒢s1trough, dims = (1,2)), dims = (1,2)) * s^-1)
...
ℰ2030smintrough = dropdims(minimum(100 * ℰs1trough, dims = 2), dims = 2)
ℰ2030smaxtrough = dropdims(maximum(100 * ℰs1trough, dims = 2), dims = 2)
ℰ2030smeantrough = dropdims(mean(100 * ℰs1trough, dims = 2), dims = 2)
bd2030strough1 = band!(panelc, TTD_time, ℰ2030smintrough[:, 1], ℰ2030smaxtrough[:, 1]; color = troughcolors[1], alpha = 0.5)
ln2030strough1 = lines!(panelc, TTD_time, ℰ2030smeantrough[:, 1]; color = troughcolors[1], linewidth = 2, linecap = :round, joinstyle = :round)
bd2030strough2 = band!(panelc, TTD_time, ℰ2030smintrough[:, 2], ℰ2030smaxtrough[:, 2]; color = troughcolors[2], alpha = 0.5)
ln2030strough2 = lines!(panelc, TTD_time, ℰ2030smeantrough[:, 2]; color = troughcolors[2], linewidth = 2, linecap = :round, joinstyle = :round)
...
bd2030strough1 = band!(paneld, TTD_time, ℰ2030smintrough[:, 1], ℰ2030smaxtrough[:, 1]; color = troughcolors[1], alpha = 0.5)
ln2030strough1 = lines!(paneld, TTD_time, ℰ2030smeantrough[:, 1]; color = troughcolors[1], linewidth = 2, linecap = :round, joinstyle = :round)
bd2030strough2 = band!(paneld, TTD_time, ℰ2030smintrough[:, 2], ℰ2030smaxtrough[:, 2]; color = troughcolors[2], alpha = 0.5)
ln2030strough2 = lines!(paneld, TTD_time, ℰ2030smeantrough[:, 2]; color = troughcolors[2], linewidth = 2, linecap = :round, joinstyle = :round)
