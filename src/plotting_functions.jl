...
function seafloorvalue(x3D, wet3D, i, j)
    ...
    return isnothing(k) ? NaN : x3D[i, j, k]
end
function seafloorvalue(x3D, wet3D)
    return [seafloorvalue(x3D, wet3D, i, j) for i in axes(x3D, 1), j in axes(x3D, 2)]
end
function seafloorvalue1D(x3D, wet3D)
    sfv = seafloorvalue(x3D, wet3D)
    return sfv[.!isnan.(sfv)]
end
...
function seafloorvalue(x3D, wet3D, gridmetrics)
    ...
    return map(eachindex(IndexCartesian(), zseafloor2D)) do I
        i, j = Tuple(I)
