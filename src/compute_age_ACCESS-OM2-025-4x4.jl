# qsub -I -P y99 -q normal -l mem=95GB -l storage=scratch/gh0+scratch/y99+scratch/p66 -l walltime=02:00:00 -l ncpus=24
...
# Coarsen 4x4 (so effectively 0.25° -> 1°)
@info "coarsening grid 4x4 everywhere"
LUMP, SPRAY, v_c = OceanTransportMatrixBuilder.lump_and_spray(wet3D, v, T; di=4, dj=4, dk=1)
M_c = LUMP * M * SPRAY
b_c = LUMP * b
...
agecube = DimensionalData.rebuild(volcello_ds["volcello"];
    ...
    dims = dims(volcello),
    metadata = Dict(
        "description" => "steady-state mean age with coarsening 4x4",
        "model" => model,
...
# Save to netCDF file
outputfile = joinpath(inputdir, "steady_age_$(κVdeep_str)_$(κH_str)_$(κVML_str)_OM2-025-4x4.nc")
