...
κs_Matt = (κVdeep = 3.0e-5, κH = 500, κVML = 0.1)
...
function labelformatterfun(E′; printat = 0.5)
    return if isapprox(E′, printat * σr; rtol = 1.0e-4)
        # "E′ = 0.5$σref"
        "RMSD = $printat$σref"
...
contour!(
    axa, rgrid' .* cos.(θgrid), rgrid' .* sin.(θgrid), E′grid;
    levels,
    linestyle = :dot,
    labels = true,
    linewidth = 1,
...
contour!(
    axa, rgrid' .* cos.(θgrid), rgrid' .* sin.(θgrid), Sgrid;
    levels = Slevels,
    labels = true,
    linewidth = 1,
    linestyle = :dash,
...
r̄ = TDvals[1].r̄
α = 0.1
isel = (df.skillscore .> 0.96) .& (-α * r̄ .< df.bias .< α * r̄) .& (-α * σr .< df.ΔSTD .< α * σr)
(sum(isel) == 1) || error
...
outputfile = joinpath(outputdir, "Taylor_diagram_3D_v3_xy_noclamp.png")
@info "Saving image file:\n  $(outputfile)"
save(outputfile, fig)
outputfile = joinpath(outputdir, "Taylor_diagram_3D_v3_xy_noclamp.pdf")
@info "Saving image file:\n  $(outputfile)"
save(outputfile, fig)
