...
#     LUMP, SPRAY, wet3D_c, v_c = OceanTransportMatrixBuilder.lump_and_spray(wet3D, v; di, dj, dk=1)
    ...
    #   Γ↑ = V⁻¹ A⁻ᵀ V 1
    v_c = LUMP * v
    V_c = sparse(Diagonal(v_c))
    V_c⁻¹ = sparse(Diagonal(1 ./ v_c))
    Γout_c = V_c⁻¹ * (transpose(A_c) \ (V_c * sΓin_c))
    Γout = SPRAY * Γout_c
    ...
    #     dims = dims(volcello_ds["volcello"]),
    #     metadata = Dict(
    #         "origin" => "mean_reemergence_time_$(di)x$(dj) computed from ACCESS-ESM1-5 historical $member Jan1990-Dec1999",
    #         "units" => "yr",
    #     )
    # )
    ...
    # outputfile = joinpath(inputdir, "mean_reemergence_time_$(di)x$(dj).nc")
    ...
    # title = "ACCESS-ESM1-5 $(di)x$(dj) historical $member Jan1990-Dec1999 reemergence time"
    # Label(fig[-1, 1:3], text = title, fontsize=20, tellwidth=false)
    ...
    title = "ACCESS-ESM1-5 $(di)x$(dj) historical $member Jan1990-Dec1999 mean age at seafloor"
    # plot options
    colorrange = (0, 1500)
    colormap = :viridis
    # plot
    ...
    # save plot
    outputfile = joinpath(inputdir, "mean_age_at_seafloor_$(di)x$(dj).png")
    ...
    title = "ACCESS-ESM1-5 $(di)x$(dj) historical $member Jan1990-Dec1999 reemergence time at seafloor"
    # plot options
    colorrange = (0, 1500)
    colormap = :viridis
    # plot
    ...
    Colorbar(fig[1,2], plt, label=rich(Γup, " at seafloor (yr)"))
    # save plot
    outputfile = joinpath(inputdir, "reemergence_time_at_seafloor_$(di)x$(dj).png")
