...
for time_window in ["Jan1850-Dec1859", "Jan1990-Dec1999", "Jan2030-Dec2039", "Jan2090-Dec2099"]
    ...
    for (irow, x3D) in enumerate((Γoutyr3D_mean, Γoutyr3D_maxdiff, 100 * Γoutyr3D_maxdiff ./ Γoutyr3D_mean))
        ...
        if irow == 1 # mean
            ...
            colorrange = extrema(levels)
            label = rich("seafloor ensemble mean ", Γup, " (yr)")
        elseif irow == 2 # max minus min
            levels = 0:50:250
            colorrange = extrema(levels)
            colormap = cgrad(:linear_bmy_10_95_c71_n256, 10, categorical = true)
            label = rich("ensemble max − min (yr)")
        elseif irow == 3 # max minus min / mean
            levels = 0:5:50
            colormap = cgrad(:linear_bmw_5_95_c86_n256, 10, categorical = true)
            colorrange = extrema(levels)
            label = rich("(max − min) / mean (%)")
        else # MLD std
            levels = 0:300:3000
            colormap = cgrad(:magma, 10, categorical = true)
            colorrange = extrema(levels)
