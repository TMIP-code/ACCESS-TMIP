...
# # Load netCDF file
# inputfile = joinpath(inputdir, "calE_centered_kVdeep3e-05_kH300_kVML1e+00.nc")
# @info "Loading monthly mean sequestration efficiency as netCDF file:\n  $(inputfile)"
# ds_montly = open_dataset(inputfile)
# ℰ_monthly = readcubedata(ds_montly.calE).data
# years_monthly = ds_montly.Ti |> Array

# inputfile = joinpath(inputdir, "seqeff_centered_kVdeep3e-05_kH300_kVML1e+00_yearly.nc")
# @info "Loading monthly mean sequestration efficiency as netCDF file:\n  $(inputfile)"
# ds_yearly = open_dataset(inputfile)
# ℰ_yearly = readcubedata(ds_yearly.seqeff).data
...
ℰij_monthly = lift(sliderij) do sliderij
    i, j = sliderij
    view(ℰ_monthly, i, j, :)
end
ℰij_yearly = lift(sliderij) do sliderij
    i, j = sliderij
    view(ℰ_yearly, i, j, :)
end
...
lines!(ax, years_monthly, ℰij_monthly; color = :blue)
lines!(ax, years_yearly, ℰij_yearly; color = :red)
...
gcdata = lift(sliderobservables[1]) do year
    viewatyear(ℰ_yearly, year)
end
colormapc = cgrad(:viridis, 10, categorical = true)
imgc = surface!(axc, lon2, lat.data, gcdata, colormap = colormapc, colorrange = (0, 1), shading = NoShading)
cc = Colorbar(gc[1, 2], imgc)
