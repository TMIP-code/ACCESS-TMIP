...
#     Γꜜᵢ = Tᵢᵢ⁻¹ 1ᵢ
# is the water age = the mean time since last surface contact.
...
prob = init(LinearProblem(Tᵢᵢ, onesᵢ), solver, rtol = 1.0e-10)
@time "Pardiso solve" Γꜜᵢ = solve!(prob).u

# Check error magnitude
@show sol_error = norm(Tᵢᵢ * Γꜜᵢ - onesᵢ) / norm(onesᵢ)

# turn the age solution vector back into a 3D cube
Γꜜyax = YAXArray(
    dims(volcello),
    ustrip.(yr, OceanTransportMatrixBuilder.as3D([zeros(sum(issrf)); Γꜜᵢ], wet3D) * s),
    Dict(
...
# Following Holzer et al. (2020) or Pasquier et al. (2024) the volume 𝒱ꜛ is given by
#     𝒱ꜛ = −Aₛ⁻¹ Vₛ Tₛᵢ Tᵢᵢ⁻¹ 1ᵢ
# But this is the same as
#     𝒱ꜛ = −Aₛ⁻¹ Vₛ Tₛᵢ Γꜜᵢ
# So I might as well compute 𝒱ꜛ now since I just computed Γꜜᵢ
...
Tₛᵢ = T[idx_surface, idx_interior]
...
# Save 𝒱ꜛ as netCDF file
𝒱ꜛ2D = fill(NaN, size(wet2D))
𝒱ꜛ2D[isurface2D] .= 𝒱ꜛ
𝒱ꜛyax = YAXArray(
    dims(areacello),
    𝒱ꜛ2D,
    Dict(
        "description" => "steady-state ocean volume ventilated up by unit area",
        "units" => "m^3/m^2",
        "solver" => "MKLPardisoIterate",
        "upwind" => upwind_str2,
    )
)
arrays = Dict(:Vup => 𝒱ꜛyax, :lat => lat, :lon => lon)
ds = Dataset(; properties = Dict(), arrays...)
# Save to netCDF file
outputfile = joinpath(inputdir, "steady_Vup_$(κVdeep_str)_$(κH_str)_$(κVML_str)_MKLPardisoIterate.nc")
@info "Saving Vup as netCDF file:\n  $(outputfile)"
savedataset(ds, path = outputfile, driver = :netcdf, overwrite = true)
