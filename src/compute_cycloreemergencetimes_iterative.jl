...
#   (I + Δt M) xₖ₊₁ = xₖ + Δt
# where M = T + Ω
# The adjoint equation for reemergence time is
#   -∂ₜx̃ + T̃ x̃ = 1 - Ω x̃
# where T̃ = V⁻¹ Tᵀ V
# Applying Euler Forward (which is the way to go since in adjoint equation time goes backwards)
#   (I + Δt M̃) xₖ₋₁ = xₖ + Δt
# where M̃ = T̃ + Ω = V⁻¹ Mᵀ V
# Note the index goes back from k to k-1 here, so that the method is implicit going back in time.
