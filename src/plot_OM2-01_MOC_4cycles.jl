...
for (ijrow, row) in enumerate(eachrow(files_df))
    ...
    # Only keep every second ρ level for cycle4 to match others
    iρ_0p125 = endswith(id, "cycle4") && (use_same_ρ_levels) ? (2:2:length(ρ_orig)) : (1:length(ρ_orig))

    ρ_skip = ρ_orig[iρ_0p125]

    # Skim some of weirdness at large densities
    iρ = ρ_skip .<= limits[4]
    ρ = ρ_skip[iρ]

    ψ = meanpsis[id].data[:, iρ_0p125[iρ]] / 1.0e9 # convert to Sv
    ...
    cycle_extra_str = cycle > 4 ? superscript("*") : ""
    cycle_extra_str_2 = " (ρ=$ρ_skip)"
    (icol == 1) && Label(fig[irow, 0], "$(resolution°[resolution]) ACCESS-OM2", tellheight = false, rotation = π / 2, font = :bold)
    (irow == 1) && Label(fig[0, icol], rich("Cycle $cycle", cycle_extra_str, cycle_extra_str_2), tellwidth = false, font = :bold)
...
imagefile = "/scratch/y99/TMIP/data/OM2-01_MOC_4cycles$(use_same_ρ_levels ? "_same_ρ_levels" : "").png"
save(imagefile, fig)
