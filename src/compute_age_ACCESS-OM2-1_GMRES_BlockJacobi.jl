...
# qsub -I -P y99 -q normal -l mem=95GB -l storage=scratch/y99+gdata/xp65 -l walltime=02:00:00 -l ncpus=24
...
# TODO : Check if Pl must be converted to CSR too!

u = ones(Nᵢ)
du = zeros(Nᵢ)
mul!(du, Tᵢᵢ, u);
@time "CSC * vec" mul!(du, Tᵢᵢ, u);

Tᵢᵢ = sparsecsr(findnz(Tᵢᵢ)..., size(Tᵢᵢ)...)

du = zeros(Nᵢ)
mul!(du, Tᵢᵢ, u);
@time "CSR * vec" mul!(du, Tᵢᵢ, u);

@time "GMRES + BlockJacobi" Γꜜᵢ, stats = Krylov.gmres(Tᵢᵢ, ones(Nᵢ);
    M = Pl,
    memory = 40,
    restart = true,
